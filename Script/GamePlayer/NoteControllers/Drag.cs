...
public partial class Drag: Sprite2D, INote
{
	[Signal]
	public delegate void DestroyDragEventHandler(); //定义信号，使用委托类型

	private Tween _tween;
	private Vector2 _endPosition;
	private double _t = 0.1f;
	/// <summary>
	/// 实例化Drag，这里还要使用接口来实现（目前还没有做）
	/// </summary>
	/// <param name="notePackedScene">Export的DragScene，在这里传入</param>
	...
	public override void _Process(double delta)
	{
		...
		if (Position.Y <= 90)
		{
			...
			{
				EmitSignal(SignalName.DestroyDrag);//发送信号
				Visible = false;
				QueueFree();
				GD.Print($"Destory");
			}
